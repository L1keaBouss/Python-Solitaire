...
# Configuration Rules of the game
from rules import *
...
# Three buttons on top side - 'New Game', 'Quit' and 'Flush All'
# One Graphic area on bottom side
layout= [[Graphwin.Button('New Game',  pad=(pad_x, (pad_y,0)), font=font),
          Graphwin.Button('Quit', pad=(pad_x, (pad_y,0)), font=font),
          Graphwin.Button('Flush All', pad=(pad_x, (pad_y,0)), font=font)],
         [Graphwin.Graph(background_color='green', canvas_size=(width, height),
          graph_bottom_left=(0,0), key='Graph', pad=(0,0), enable_events=True,
          graph_top_right=(width, height), drag_submits=True)]]

# Create window
win  = Graphwin.Window('Solitaire Game', layout=layout,
...
while True:
    ...
    if New_Start:
        card_list = random.sample(range(NUMBER_OF_CARDS), NUMBER_OF_CARDS)
        p = [0 for i in range(all)]
        for i in range(cells+NUMBER_OF_CARDS):    # Initial cards
            kind = 'Blank Card' if i < cells else 'Card'
            p[i] = SolitaireApp(card=i, kind=kind)
        p[0].flash()                    # Flash cards
        for card in show_card: p[card].turn_over()
        New_Start = False

    event, values = win.read(timeout=100)    # read window event

    if event==None or event=='Quit':   # Window close when Quit clicked
        break

    # No event
    elif event == Graphwin.TIMEOUT_KEY:
        continue
